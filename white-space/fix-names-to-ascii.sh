...
# this script is a wrapper for mp3gain
# it normalize all mp3 files in the current directory
...
function namefix(){
    ...
    temp=$1
    for (( i=0; i<${#acc[@]}; i++ )); do
        temp=`echo $temp | sed "s/${acc[$i]}/${noa[$i]}/g"`

    done
    echo $temp | tr '[:upper:]' '[:lower:]' | tr -s ' ' | tr -s '_' | tr -s '-' | tr -d ',' | tr -d '(' | tr -d ')'| tr -d '’' | tr -d "'"  | tr -d '' | rev | cut -c2- | rev 
}

function whitespace(){
    declare temp
    temp=$1
    temp=`echo $temp | tr -s ' ' | tr ' ' '-'`
    echo $temp
}

function rename(){
    if [[ debug -eq 1 ]]; then
        printf "rename \033[38;5;124m$1 \033[38;5;46m$2\033[0m\n"
    else
        mv "$1" "$2"
    fi
